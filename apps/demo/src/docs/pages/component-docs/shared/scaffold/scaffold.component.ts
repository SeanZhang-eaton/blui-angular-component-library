...
export class ScaffoldComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() md: string;
  @Input() examplesRef: TemplateRef<unknown>;
  @Input() playgroundRef: TemplateRef<unknown>;
  ...
  getKeys(knobs: { [key: string]: Knob }): string[] {
    return Object.keys(knobs || {}).sort();
  }

  ngOnInit(): void {
    if (this.mdFileName) {
      this._markdownService.getSource(`assets/md/${this.mdFileName}`).subscribe((data) => {
        let adjusted = data;
        adjusted = adjusted.replace(/images/g, `assets/md/images/`);
        adjusted = adjusted.replace(/gifs/g, `assets/md/gifs/`);
